...
function NJTAApp({ savedReport, setSavedReport, tier, authToken }) {
  ...
  // Dev/testing shortcut — jumps straight to a random profile result so post-result
  // features (portraits, shadow readouts, etc.) can be tested without retaking the
  // full 56-question assessment each time.
  function goToRandomProfile() {
    const typeNames = Object.keys(ALL_TYPES);
    const processes = ["Eudemonic", "Elliptic", "Hyperbolic", "Psychotic"];
    ...
    setProcResult(randomProcess);
    setPhase("result");
    setRetakeConfirm(false);
    if (setSavedReport) setSavedReport({ typeName: randomType, process: randomProcess });
    persistReport({ typeName: randomType, process: randomProcess });
  }
    ...
    <div style={{minHeight:"calc(100vh - 52px)",background:"radial-gradient(ellipse at 30% 20%, #1A0A20 0%, #0A0A14 50%, #050510 100%)",padding:"24px 14px",fontFamily:"'Playfair Display',Georgia,serif",color:"#E8DFC8"}}>
      ...
      {phase==="intro" && (
        <div style={{maxWidth:"560px",margin:"0 auto",textAlign:"center",paddingTop:"28px",animation:"fadeIn 0.4s ease"}}>
          ...
          <div>
            <button onClick={goToRandomProfile} style={{marginTop:"14px",background:"transparent",border:"1px solid #3A3A4A",color:"#7A7A8A",padding:"10px 28px",fontSize:"11px",letterSpacing:"2px",textTransform:"uppercase",fontFamily:"Inter,sans-serif",fontWeight:"500",borderRadius:"4px",cursor:"pointer"}}>Go to Profile</button>
          </div>
        </div>
      )}
      ...
      {phase==="result" && result && procResult && (
        <div style={{maxWidth:"680px",margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:"28px"}}>
            ...
            <div style={{color:"#6A6A8A",fontSize:"12px",fontFamily:"Inter,sans-serif",marginTop:"10px",letterSpacing:"2px",textTransform:"uppercase"}}>Assessment Complete</div>
          </div>
...
function DreamApp({ savedEntries = [], setSavedEntries, tier, chosenAnalystId, setChosenAnalystId, authToken }) {
  // Choose Your Analyst is always shown on entry, even for a returning Tier 2 user whose
  // choice is already locked in — seeing the other four analysts greyed out (rather than
  // skipping straight past them) is itself part of the upsell toward Tier 3.
          ...
          <>
            {step==="intro" && (
              <div style={{animation:"daseinFade 0.5s ease"}}>
                ...
                <div style={{marginTop:24,paddingTop:20,borderTop:`1px dashed ${DASEIN_PALETTE.ash}`}}>
                  <button onClick={()=>{setPreAboutStep("intro"); setStep("about");}} style={{background:"transparent",border:`1px solid ${DASEIN_PALETTE.flare}55`,color:DASEIN_PALETTE.flare,fontFamily:"sans-serif",fontSize:11,letterSpacing:"0.1em",cursor:"pointer",padding:"8px 16px",borderRadius:4}}>[dev] Preview About Page →</button>
                </div>
              </div>
            )}
...
// POST /api/signin, both returning { token, user }). Reads VITE_BACKEND_URL at build
// time so this can point at production without editing source; falls back to the local
// dev server since this is the dev build. Set VITE_BACKEND_URL=https://api.intraself.net/api
// in Vercel's env vars for any deploy that should hit the real backend.
...
export default function IntraSelf() {
  ...
  // Dev-only tier switcher for QA — lets any tier be tested without real payment flow.
  // Overrides the computed tier above when set; null means "use the real computed tier."
  ...
  const devTierSwitcher = (
    <div style={{ position:"fixed", bottom:14, right:14, zIndex:9999, display:"flex", gap:4, background:"#0D0D14", border:"1px solid #2A2A38", borderRadius:20, padding:4, fontFamily:"sans-serif" }}>
      <span style={{ color:"#4A4A5A", fontSize:10, padding:"4px 6px", alignSelf:"center" }}>[dev tier]</span>
      {[1,2,3].map(t=>(
        <button key={t} onClick={()=>setDevTierOverride(t)} style={{ width:24, height:24, borderRadius:"50%", border:"none", cursor:"pointer", fontSize:11, background:effectiveTier===t?"#C8A84B":"#1A1A24", color:effectiveTier===t?"#0A0A14":"#6A6A7A" }}>{t}</button>
      ))}
    </div>
  );

  if (!active) return (<>
    <HomePage onSelect={handleSelect} user={currentUser} onAccountClick={()=>setAccountView(currentUser ? "profile" : "auth")} />
    {devTierSwitcher}
  </>);
  return (<>
    <AppShell active={active} onHome={() => setActive(null)}>
      {active === "njta"   && <NJTAApp savedReport={njtaReport} setSavedReport={setNjtaReport} tier={effectiveTier} authToken={authToken} />}
      {active === "dream"  && <DreamApp savedEntries={dreamEntries} setSavedEntries={setDreamEntries} tier={effectiveTier} chosenAnalystId={currentUser?.chosenAnalystId} setChosenAnalystId={(id)=>updateUser(u=>u?{...u, chosenAnalystId:id}:u)} authToken={authToken} />}
      {active === "dasein" && <DaseinApp savedSessions={daseinSessions} setSavedSessions={setDaseinSessions} tier={effectiveTier} authToken={authToken} />}
    </AppShell>
    {devTierSwitcher}
  </>);
}
