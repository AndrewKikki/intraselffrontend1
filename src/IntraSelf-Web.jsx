...
export default function IntraSelf() {
  ...
  return (<>
    <AppShell active={active} onHome={() => setActive(null)}>
      {active === "njta"   && <NJTAApp savedReport={njtaReport} setSavedReport={syncedSetNjtaReport} tier={effectiveTier} />}
      {active === "dream"  && <DreamApp savedEntries={dreamEntries} setSavedEntries={syncedSetDreamEntries} tier={effectiveTier} chosenAnalystId={currentUser?.chosenAnalystId} setChosenAnalystId={(id)=>updateUser(u=>u?{...u, chosenAnalystId:id}:u)} />}
      {active === "dasein" && <DaseinApp savedSessions={daseinSessions} setSavedSessions={syncedSetDaseinSessions} tier={effectiveTier} />}
    </AppShell>
    {devTierSwitcher}
  </>);
}
